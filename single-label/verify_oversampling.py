...
def verify_per_aspect_balance(csv_file):
    ...
    for aspect in aspects:
        ...
        if len(unique_counts) == 1:
            logger.info(f"  [OK] BALANCED - All sentiments have {unique_counts[0]} samples")
        else:
            logger.info(f"  [X] IMBALANCED - Counts: {sentiment_counts.tolist()}")
            all_balanced = False
    ...
    if all_balanced:
        logger.info("[PASS] ALL ASPECTS ARE BALANCED!")
    else:
        logger.info("[WARN] SOME ASPECTS ARE IMBALANCED")
