...
def analyze_imbalance(df, aspect_cols):
    ...
    print(f"\n{'Aspect':<15} {'Positive':<10} {'Negative':<10} {'Neutral':<10} {'Empty':<10} {'Imbalance':<10}")
    print("-" * 75)
    
    for aspect in aspect_cols:
        # Count each sentiment (case-insensitive)
        aspect_series = df[aspect].fillna('').str.lower()
        counts = aspect_series.value_counts()
        ...
        print(f"{aspect:<15} {pos:<10} {neg:<10} {neu:<10} {empty:<10} {imbalance_ratio:<10.2f}x")
...
def oversample_simple_per_aspect(df, aspect_cols, seed=42):
    ...
    for aspect in aspect_cols:
        ...
        for sentiment in ['Positive', 'Negative', 'Neutral']:
            ...
            # Get rows with this sentiment for this aspect (case-insensitive)
            mask = df[aspect].str.lower() == sentiment.lower()
...
def main(config_path: Optional[str] = None):
    ...
    if config_path:
        ...
        input_file = 'multi-label/data/train.csv'
        output_file = 'multi-label/data/train_balanced.csv'
        seed = 42
    ...
    print(f"   train_file: data/train_balanced.csv")
    print(f"   Then train: python train_multilabel.py")
