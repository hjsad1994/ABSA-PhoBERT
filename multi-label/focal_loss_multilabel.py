...
class MultilabelFocalLoss(nn.Module):
    ...
    Args:
        alpha (Union[List[float], torch.Tensor], optional): Class weights for 
            [negative, neutral, positive, none]. If None, no class weighting is applied.
    ...
    Example:
        >>> focal_loss = MultilabelFocalLoss(alpha=[0.8, 1.0, 2.5, 1.0], gamma=2.0)
        >>> logits = torch.randn(16, 11, 4, requires_grad=True)
    ...
    def forward(
        ...
        # Create mask for valid labels (ignore class 3 = "none")
        # Only compute loss for aspects with real sentiment (0=Negative, 1=Neutral, 2=Positive)
        valid_mask = (target_flat != 3)
        
        if valid_mask.sum() == 0:
            # No valid labels in batch (all "none")
        ...
        log_pt = log_probs.gather(dim=1, index=target_valid.unsqueeze(1)).squeeze(1)
        pt = probs.gather(dim=1, index=target_valid.unsqueeze(1)).squeeze(1)
        
        # Compute focal weight: (1 - p_t)^gamma
        focal_weight = (1.0 - pt).pow(self.gamma)
        
        # Apply alpha weighting (only for valid classes 0, 1, 2)
        alpha_t = self.alpha[target_valid]
        
        # Focal loss: -alpha_t * (1 - p_t)^gamma * log(p_t)
        loss = -alpha_t * focal_weight * log_pt
        
        # Apply reduction (average over valid positions only)
        if self.reduction == 'none':
            # Scatter back to original positions
            full_loss = torch.zeros(batch_size * num_aspects, device=input.device)
            full_loss[valid_mask] = loss
            return full_loss.view(batch_size, num_aspects)
        elif self.reduction == 'mean':
            return loss.mean()  # Average over valid positions only
        else:  # sum
            return loss.sum()
...
def calculate_global_alpha(
    ...
    Returns:
        List of alpha weights ordered as [negative_weight, neutral_weight, positive_weight, none_weight]
    ...
    # Collect all sentiments from all aspects (including empty as 'none')
    all_sentiments = []
    
    for aspect in aspect_cols:
        if aspect in df.columns:
            sentiments = df[aspect].fillna('').astype(str).str.strip().str.lower()
            # Convert empty strings to 'none'
            sentiments = sentiments.replace('', 'none')
...
def _test_focal_loss():
    ...
    focal_loss = MultilabelFocalLoss(alpha=[1.0, 1.0, 1.0, 1.0], gamma=2.0)
    loss = focal_loss(logits, labels)
    loss.backward()
    ...
    focal_loss_weighted = MultilabelFocalLoss(alpha=[0.8, 1.0, 2.5, 1.0], gamma=2.0)
    ...
    if torch.cuda.is_available():
        ...
        focal_gpu = MultilabelFocalLoss(alpha=[0.8, 1.0, 2.5, 1.0], gamma=2.0).to(device)
