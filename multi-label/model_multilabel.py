"""
Multi-Label ABSA Model for Vietnamese Sentiment Analysis using PhoBERT
Predicts all 11 aspects × 4 classes (Negative, Neutral, Positive, None) in one forward pass
"""

import os
# Workaround for PyTorch < 2.6 security check (PhoBERT uses safetensors)
...
class MultiLabelPhoBERT(nn.Module):
    """
    Multi-Label ABSA Model using PhoBERT
    
    Input:  "Pin trâu camera xấu"
    Output: Battery=positive, Camera=negative, Performance=none, ... (11 aspects)
           Each aspect has 4 classes: Negative, Neutral, Positive, None (no sentiment)
    """
    ...
    def __init__(
        ...
        # BERT encoder (use safetensors to avoid PyTorch version check)
        try:
        ...
        # Output: 11 aspects × 4 sentiments = 44 classes
        ...
        # Sentiment names (matching dataset_multilabel.py mapping)
        self.sentiment_names = ['negative', 'neutral', 'positive', 'none']
